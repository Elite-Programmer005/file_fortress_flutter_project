...
class AllFilesTab extends StatelessWidget {
  ...
  Future<void> _restoreFile(BuildContext context, FileEntity fileEntity) async {
    ...
    try {
      ...
      final restoreDir = await _resolveRestoreDirectory(fileEntity);
      if (!await restoreDir.exists()) {
        await restoreDir.create(recursive: true);
      }

      final restoredPath = _uniqueRestorePath(
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                Expanded(
                    child: Text(
                        '${fileEntity.originalName} restored to ${path.basename(restoreDir.path)}')),
              ],
            ),
  ...
  Future<Directory> _resolveRestoreDirectory(FileEntity fileEntity) async {
    if (Platform.isAndroid) {
      if (fileEntity.fileType.startsWith('image')) {
        return Directory('/storage/emulated/0/Pictures');
      }
      if (fileEntity.fileType.startsWith('video')) {
        return Directory('/storage/emulated/0/Movies');
      }
      if (fileEntity.fileType.startsWith('audio')) {
  ...
  String _uniqueRestorePath({
    required Directory restoreDir,
    required String originalName,
  }) {
    final baseName = path.basenameWithoutExtension(originalName);
    final ext = path.extension(originalName);
    var candidate = path.join(restoreDir.path, originalName);
    var counter = 1;

    while (File(candidate).existsSync()) {
      candidate = path.join(restoreDir.path, '$baseName ($counter)$ext');
      counter++;
    }
  ...
  void _confirmDelete(BuildContext context, FileEntity file) {
    ...
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: const Text(
            "This action cannot be undone. The file will be permanently removed from the vault."),
        actions: [
          TextButton(
          ...
          TextButton(
            onPressed: () async {
              ...
              await _deletePermanently(rootContext, file);
            },
  ...
  Future<void> _deletePermanently(BuildContext context, FileEntity file) async {
    final encryptedFile = File(file.encryptedPath);
    try {
      if (await encryptedFile.exists()) {
        await encryptedFile.delete();
        if (await encryptedFile.exists()) {
          throw Exception('Unable to remove file from device storage');
        }
      }

      await Hive.box<FileEntity>('files').delete(file.id);
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return Card(
              ...
              child: ListTile(
                leading: Icon(_getIconForFileType(file.fileType),
                ...
                trailing: IconButton(
                  ...
                  onPressed: () => showFileOptions(context, file),
