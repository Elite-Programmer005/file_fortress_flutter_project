  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder(
      ...
      builder: (context, Box<FileEntity> box, _) {
        ...
        final files = box.values.where((file) {
          final type = file.fileType.toLowerCase();
          return !type.startsWith('image/') && 
                 !type.startsWith('video/') && 
                 !type.startsWith('audio/');
        }).toList().cast<FileEntity>();

        if (files.isEmpty) return const Center(child: Text('No documents in vault'));
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return Card(
              ...
              child: ListTile(
                leading: const Icon(Icons.description, color: Colors.blueGrey, size: 30),
